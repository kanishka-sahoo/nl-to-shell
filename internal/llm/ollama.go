...
func (p *OllamaProvider) validateResultInternal(ctx context.Context, command, output, intent string) (*types.ValidationResponse, error) {
	...
	request := OllamaRequest{
		Model:  p.getModel(),
		Prompt: "You are an expert system administrator. Analyze command execution results and determine if they match the user's intent. Respond with a JSON object containing 'is_correct' (boolean), 'explanation' (string), 'suggestions' (array of strings), and 'correction' (string if needed).\n\n" + validationPrompt,
		Stream: false,
		Options: &OllamaOptions{
			Temperature: 0.1,
			NumPredict:  300,
...
func (p *OllamaProvider) buildSystemPrompt(context *types.Context) string {
...
func (p *OllamaProvider) buildValidationPrompt(command, output, intent string) string {
